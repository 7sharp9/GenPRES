...
module Constraint =
    ...
    let orderConstraints log cs =
        ...
        |> List.sortBy fst
        |> fun cs ->
            cs |> Events.ConstraintSortOrder |> Logging.logInfo log
    ...
    let apply onlyMinIncrMax log (c: Constraint) eqs =
        ...
        |> function
            ...
            | vr :: _ -> c.Property |> Property.toValueRange |> Variable.setValueRange onlyMinIncrMax vr
        |> fun var ->
            c |> Events.ConstraintApplied |> Logging.logInfo log
    ...
    let solve onlyMinIncrMax log sortQue (c: Constraint) eqs =
        ...
        |> fun eqs ->
            c |> Events.ConstrainedSolved |> Logging.logInfo log
