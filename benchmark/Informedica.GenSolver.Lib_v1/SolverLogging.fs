...
module SolverLogging =
    ...
    let rec printException =
        function
        | Exceptions.ValueRangeEmptyValueSet -> "ValueRange cannot have an empty value set"

        | Exceptions.EquationEmptyVariableList -> "An equation should at least contain one variable"

        | Exceptions.SolverInvalidEquations eqs -> $"The following equations are invalid {eqs |> eqsToStr} "
...
"""
    ...
    let printMsg =
        function
        | ExceptionMessage m -> m |> printException
        | SolverMessage m ->
            let toString eq =
                ...
                | [] -> ""
                | [ _ ] -> ""
                | y :: xs -> $"""{y |> varName} = {xs |> List.map varName |> String.concat op}"""


            match m with
            | EquationStartedSolving eq -> $"=== Start solving Equation ===\n{eq |> toString}"
...
"""
            ...
            | SolverStartSolving eqs -> $"=== Solver Start Solving ===\n{eqs |> eqsToStr}"

            | SolverLoopedQue(n, eqs) -> $"solver looped que {n} times with {eqs |> List.length} equations"
    ...
    let logger f =
        {
            Log =
                fun
                    ...
                    match msg with
                    | :? Logging.SolverMessage as m -> m |> printMsg |> f
