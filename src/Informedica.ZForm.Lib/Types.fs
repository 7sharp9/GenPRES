...
module Types =
    ...
    type PatientCategory =
        ...
        static member GestAge_: (PatientCategory -> MinMax) * (MinMax -> PatientCategory -> PatientCategory) =
            _.GestAge, (fun a p -> { p with GestAge = a })

        static member Age_: (PatientCategory -> MinMax) * (MinMax -> PatientCategory -> PatientCategory) =
            _.Age, (fun a p -> { p with Age = a })

        static member Weight_: (PatientCategory -> MinMax) * (MinMax -> PatientCategory -> PatientCategory) =
            _.Weight, (fun w p -> { p with Weight = w })

        static member BSA_: (PatientCategory -> MinMax) * (MinMax -> PatientCategory -> PatientCategory) =
            _.BSA, (fun b p -> { p with BSA = b })

        static member Gender_: (PatientCategory -> Gender) * (Gender -> PatientCategory -> PatientCategory) =
    ...
    type DoseRange =
        ...
        static member Norm_: (DoseRange -> MinMax) * (MinMax -> DoseRange -> DoseRange) =
            _.Norm, (fun mm dr -> { dr with Norm = mm })

        static member NormWeight_: (DoseRange -> MinMax * Unit) * (MinMax * Unit -> DoseRange -> DoseRange) =
            _.NormWeight, (fun mm dr -> { dr with NormWeight = mm })

        static member NormBSA_: (DoseRange -> MinMax * Unit) * (MinMax * Unit -> DoseRange -> DoseRange) =
            _.NormBSA, (fun mm dr -> { dr with NormBSA = mm })

        static member Abs_: (DoseRange -> MinMax) * (MinMax -> DoseRange -> DoseRange) =
            _.Abs, (fun mm dr -> { dr with Abs = mm })

        static member AbsWeight_: (DoseRange -> MinMax * Unit) * (MinMax * Unit -> DoseRange -> DoseRange) =
            _.AbsWeight, (fun mm dr -> { dr with AbsWeight = mm })

        static member AbsBSA_: (DoseRange -> MinMax * Unit) * (MinMax * Unit -> DoseRange -> DoseRange) =
            _.AbsBSA, (fun mm dr -> { dr with AbsBSA = mm })
    ...
    type Dosage =
        ...
        static member Name_: (Dosage -> string) * (string -> Dosage -> Dosage) =
            _.Name, (fun s d -> { d with Name = s })

        static member StartDosage_: (Dosage -> DoseRange) * (DoseRange -> Dosage -> Dosage) =
            _.StartDosage, (fun dr d -> { d with StartDosage = dr })

        static member SingleDosage_: (Dosage -> DoseRange) * (DoseRange -> Dosage -> Dosage) =
            _.SingleDosage, (fun dr d -> { d with SingleDosage = dr })

        static member RateDosage_: (Dosage -> DoseRange * RateUnit) * (DoseRange * RateUnit -> Dosage -> Dosage) =
            _.RateDosage, (fun dr d -> { d with RateDosage = dr })

        static member TotalDosage_: (Dosage -> DoseRange * Frequency) * (DoseRange * Frequency -> Dosage -> Dosage) =
            _.TotalDosage, (fun dt d -> { d with TotalDosage = dt })
    ...
    and Frequency =
        ...
        static member Frequencies_: (Frequency -> Frequencies) * (Frequencies -> Frequency -> Frequency) =
            _.Frequencies, (fun frs fr -> { fr with Frequencies = frs })

        static member TimeUnit_: (Frequency -> Unit) * (Unit -> Frequency -> Frequency) =
            _.TimeUnit, (fun tu fr -> { fr with TimeUnit = tu })

        static member MinimalInterval_: (Frequency -> ValueUnit Option) * (ValueUnit Option -> Frequency -> Frequency) =
            _.MinimalInterval, (fun mi fr -> { fr with MinimalInterval = mi })
    ...
    and Rule =
        | GStandRule of string
        | PedFormRule of string
    ...
    type PatientDosage =
        ...
        static member Patient_: (PatientDosage -> PatientCategory) * (PatientCategory -> PatientDosage -> PatientDosage) =
            _.Patient, (fun pat pd -> { pd with Patient = pat })

        static member FormDosage_: (PatientDosage -> Dosage) * (Dosage -> PatientDosage -> PatientDosage) =
            _.FormDosage, (fun sd pd -> { pd with FormDosage = sd })

        static member SubstanceDosages_
            : (PatientDosage -> Dosage list) * (Dosage list -> PatientDosage -> PatientDosage) =
            _.SubstanceDosages, (fun d sd -> { sd with SubstanceDosages = d })
    ...
    type TradeProductLabel =
        ...
        static member HPK_: (TradeProductLabel -> int) * (int -> TradeProductLabel -> TradeProductLabel) =
            _.HPK, (fun hpk tp -> { tp with HPK = hpk })


        static member Label_: (TradeProductLabel -> string) * (string -> TradeProductLabel -> TradeProductLabel) =
            _.Label, (fun lbl tp -> { tp with Label = lbl })
    ...
    type GenericProductLabel =
        ...
        static member GPK_: (GenericProductLabel -> int) * (int -> GenericProductLabel -> GenericProductLabel) =
            _.GPK, (fun hpk tp -> { tp with GPK = hpk })


        static member Label_: (GenericProductLabel -> string) * (string -> GenericProductLabel -> GenericProductLabel) =
            _.Label, (fun lbl tp -> { tp with Label = lbl })
    ...
    type RouteDosage =
        ...
        static member Route_: (RouteDosage -> string) * (string -> RouteDosage -> RouteDosage) =
            _.Route, (fun s rd -> { rd with Route = s })

        static member FormDosages_: (RouteDosage -> FormDosage list) * (FormDosage list -> RouteDosage -> RouteDosage) =
            _.FormDosages, (fun pdl rd -> { rd with FormDosages = pdl })
    ...
    type DoseRule =
        ...
        static member Generic_: (DoseRule -> string) * (string -> DoseRule -> DoseRule) =
            _.Generic, (fun s dr -> { dr with Generic = s })

        static member Synonyms_: (DoseRule -> string list) * (string list -> DoseRule -> DoseRule) =
            _.Synonyms, (fun sns dr -> { dr with Synonyms = sns |> List.distinct })


        static member IndicationDosages_
            : (DoseRule -> IndicationDosage list) * (IndicationDosage list -> DoseRule -> DoseRule) =
            _.IndicationsDosages, (fun inds dr -> { dr with IndicationsDosages = inds })
