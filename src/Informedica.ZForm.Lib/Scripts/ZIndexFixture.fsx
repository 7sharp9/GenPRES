...
module ZIndexFixture =
    ...
    let padL (n: int) (s: string) =
        if s.Length >= n then s.Substring(s.Length - n, n) else s.PadLeft(n)
    ...
    let private bst001Record bstNum mdbst mdvnr mdrnam mdroms mdrtyp mdrlen mdrdec mdropm =
        padL 4 bstNum         +   // BSTNUM:  4
        "0"                   +   // MUTKOD:  1
        padR 20 mdbst         +   // MDBST:  20
        padL 3 (string mdvnr) +   // MDVNR:   3
        padR 10 mdrnam        +   // MDRNAM: 10
        padR 50 mdroms        +   // MDROMS: 50
        "        "            +   // MDRCOD:  8
        "  "                  +   // MDRSLE:  2
        padR 1 mdrtyp         +   // MDRTYP:  1
        padL 4 (string mdrlen) +  // MDRLEN:  4
        padL 2 (string mdrdec) +  // MDRDEC:  2
        padR 6 mdropm         +   // MDROPM:  6
        padR 17 ""                // padding: 17
    ...
    let private tableDefs bstNum bstName (fields: (string * string * string * int * int * string) list) =
        fields
        |> List.mapi (fun i (nm, ds, tp, ln, dc, fm) ->
            bst001Record bstNum bstName (i + 1) nm ds tp ln dc fm
        )
    ...
    let allBst001TLines =
        [ bst020TDefs; bst360TDefs; bst380TDefs; bst640TDefs; bst641TDefs
          bst642TDefs; bst643TDefs; bst649TDefs; bst701TDefs; bst711TDefs
          bst715TDefs; bst720TDefs; bst725TDefs; bst750TDefs; bst760TDefs
          bst801TDefs; bst902TDefs; bst922TDefs ]
        |> List.concat
    ...
    let private bst649TLen  = [| 4; 1; 6; 11; 11; 11; 11; 11; 11; 11; 11; 11; 11; 11; 11 |] // total=143
    let private bst701TLen  = [| 4; 1; 8; 4; 1; 6; 12; 4; 4; 6; 4; 4 |]  // total=58
    let private bst711TLen  = [| 4; 1; 8; 8; 4; 5; 4; 5; 7; 4; 4; 5; 6; 4; 4; 4; 4; 8; 4; 4; 7; 4; 5 |] // total=123
    ...
    let private buildRecord (lens: int[]) (vals: string[]) =
        ...
        Array.map2 (fun (len: int) (v: string) ->
            if v.Length <= len then v.PadRight(len)
            else v.Substring(0, len)
        ) lens vals
        |> String.concat ""
    ...
    let private bst020Record nmnr nmmemo nmetik nmnm40 nmnaam =
        buildRecord bst020TLen [|
            "0020"; "0"
            (string nmnr).PadLeft(7); padR 3 nmmemo
            padR 25 nmetik; padR 40 nmnm40; padR 60 nmnaam
        |]

    let private bst360Record ttehnr ttehom =
        buildRecord bst360TLen [|
            "0360"; "0"
    ...
    let private bst640Record gpkode gpdgst gprisc =
        buildRecord bst640TLen [|
            "0640"; "0"; id71 gpkode
            "    "; "    "; "    "
            (string gpdgst); "    "; padR 2 gprisc
        |]

    let private bst641Record gpkode prkode hpkode gpdcth gpdcod gpdbas =
        buildRecord bst641TLen [|
            "0641"; "0"
            id71 gpkode; id71 prkode; id71 hpkode
            (string gpdcth).PadLeft(4); (string gpdcod).PadLeft(4)
            (string gpdbas).PadLeft(6)
        |]

    let private bst642Record gpdbas gpdid1 gpdzco icpcnr1 icpcto gpktth gpktwg gpdcat =
        buildRecord bst642TLen [|
            "0642"; "0"
            (string gpdbas).PadLeft(6); (string gpdid1).PadLeft(6)
            "    "; (string gpdzco).PadLeft(5); (string icpcnr1).PadLeft(6)
            "    "; (string icpcto).PadLeft(2)
            "    "; "    "
            (string gpktth).PadLeft(5); (string gpktwg).PadLeft(5)
    ...
    let private bst643Record gpdcat gpdid2 gpdlfm gpdlfx gpdkgm gpdkgx gpdm2m gpdm2x gpdfaa gpdfee gpdden gpddnr =
        buildRecord bst643TLen [|
            "0643"; "0"
            (string gpdcat).PadLeft(6); (string gpdid2).PadLeft(6)
            fmtD42 gpdlfm; fmtD42 gpdlfx
            fmtD33 gpdkgm; fmtD33 gpdkgx
            fmtD33 gpdm2m; fmtD33 gpdm2x
            fmtD22 gpdfaa; (string gpdfee).PadLeft(4)
            padR 1 gpdden; (string gpddnr).PadLeft(6)
        |]

    let private bst649Record gpddnr nrmmin nrmmax absmin absmax nrmmink nrmmaxk absmink absmaxk nrmminm nrmmaxm absminm absmaxm =
        buildRecord bst649TLen [|
            "0649"; "0"; (string gpddnr).PadLeft(6)
            fmtD83 nrmmin;  fmtD83 nrmmax
            fmtD83 absmin;  fmtD83 absmax
            fmtD83 nrmmink; fmtD83 nrmmaxk
            fmtD83 absmink; fmtD83 absmaxk
            fmtD83 nrmminm; fmtD83 nrmmaxm
            fmtD83 absminm; fmtD83 absmaxm
        |]

    let private bst701Record hpkode gnvolg gnmwhs gngnk gnminh thmine xnmine gnstam thstwg ssktwg =
        buildRecord bst701TLen [|
            "0701"; "0"; id71 hpkode
            (string gnvolg).PadLeft(4); padR 1 gnmwhs
            id51 gngnk; fmtD93 gnminh
            (string thmine).PadLeft(4); (string xnmine).PadLeft(4)
            id51 gnstam
            (string thstwg).PadLeft(4); (string ssktwg).PadLeft(4)
        |]

    let private bst711Record gpkode gskode gpktvr gpktwg gpnmnr gpmlci gpmlct spkode atcode xpehhv =
        buildRecord bst711TLen [|
            "0711"; "0"; id71 gpkode; id71 gskode
            "    "; (string gpktvr).PadLeft(5)
            "    "; (string gpktwg).PadLeft(5)
            (string gpnmnr).PadLeft(7)
            "    "; "    "
            (string gpmlci).PadLeft(5); (string gpmlct).PadLeft(6)
            "    "; "    "; "    "; "    "
            id71 spkode
            "    "; "    "
            padR 7 atcode
    ...
    let private bst715Record gnmwhs gskode gnnkpk gnmomh xnmome xpehhv =
        buildRecord bst715TLen [|
            "0715"; "0"; padR 1 gnmwhs; id71 gskode
            id51 gnnkpk; fmtD93 gnmomh
            (string xnmome).PadLeft(4); (string xpehhv).PadLeft(5)
        |]
    ...
    let private bst725Record sskode gnstam ssktwg =
        buildRecord bst725TLen [|
            "0725"; "0"; id71 sskode; id51 gnstam
            (string ssktwg).PadLeft(5)
        |]

    let private bst750Record gngnk gngnam gnstam gnnkpk gnstnt gnwzhs gnstkd gncas gnform gnmole gnmoli gnmols gnsgew gnvoor =
        buildRecord bst750TLen [|
            "0750"; "0"; id51 gngnk; padR 40 gngnam
            id51 gnstam; id51 gnnkpk
            padR 1 gnstnt; padR 1 gnwzhs
            (string gnstkd).PadLeft(6); id81 gncas
            padR 20 gnform; fmtD84 gnmole
            padR 1 gnmoli; fmtD84 gnmols; fmtD25 gnsgew
            padR 6 gnvoor
        |]

    let private bst760Record hpkode enktdw =
        buildRecord bst760TLen [|
            "0760"; "0"; id71 hpkode; "    "; "    "
            (string enktdw).PadLeft(5)
        |]

    let private bst801Record atcode atoms atomse atkind =
        buildRecord bst801TLen [|
            "0801"; "0"; padR 7 atcode; padR 60 atoms; padR 60 atomse; padR 1 atkind
        |]

    let private bst902Record tsnr tsitnr thnm25 thnm50 =
        buildRecord bst902TLen [|
            "0902"; "0"
            (string tsnr).PadLeft(4); (string tsitnr).PadLeft(6)
            "    "; "     "; "          "
            padR 25 thnm25; padR 50 thnm50
        |]

    let private bst922Record thmodu txmodu thtsrt txtsrt txkode txblnr txrgln txtext =
        buildRecord bst922TLen [|
            "0922"; "0"
            (string thmodu).PadLeft(4); (string txmodu).PadLeft(6)
            (string thtsrt).PadLeft(4); (string txtsrt).PadLeft(6)
            (string txkode).PadLeft(8)
            (string txblnr).PadLeft(6); (string txrgln).PadLeft(6)
            padR 200 txtext
        |]
    ...
    let private icpcRecords = [
        bst380Record 0 "alle indicaties"
    ]
    ...
    let private textBlockRecords = [
        bst922Record 103 1 104 1 694554 1 1 "<p>Paracetamol is een veelgebruikt pijnstiller en koortsverlagend middel.</p>"
