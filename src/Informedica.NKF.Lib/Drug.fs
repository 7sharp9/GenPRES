...
module Drug =
    ...
    module Frequency =
        ...
        let isValid =
            function
            | Frequency fr
            | PRN fr -> fr.Max > 0 && fr.Time > 0 && fr.Unit |> String.notEmpty
            | _ -> true
        ...
        let getFrequency freq =
            match freq with
            | PRN qty
            | Frequency qty -> [ qty.Min .. 1 .. qty.Max ] |> List.map string |> String.concat ";"
            | _ -> ""
    ...
    module Target =
        ...
        let getAgeInDays target =
            let toDays v u =
                ...
                |> Option.bind (fun u ->
                    ...
                    |> Option.bind (fun br ->
                        ...
                        |> ValueUnit.getValue
                        |> function
                            | [| days |] -> days |> BigRational.toFloat |> Some
                            | _ -> None
                    )
                )
            ...
            |> getAge
            |> function
                | Some min, Some max -> toDays min.Quantity min.Unit, toDays max.Quantity max.Unit
                | Some min, None -> toDays min.Quantity min.Unit, None
                | None, Some max -> None, toDays max.Quantity max.Unit
                | None, None ->
        ...
        let getWeightInGram target =
            ...
            |> getWeight
            |> function
                | Some min, Some max -> toGram min.Quantity min.Unit, toGram max.Quantity max.Unit
                | Some min, None -> toGram min.Quantity min.Unit, None
                | None, Some max -> None, toGram max.Quantity max.Unit
                | None, None -> None, None
    ...
    let mapDrug (drug: Drug) =
        ...
        |> Array.filter (fun pm -> pm.medication |> String.equalsCapInsens drug.Generic)
        |> Array.toList
        |> function
            | [] -> [ drug ]
            | pms ->
                ...
                |> List.map (fun pm ->
                    let drug =
                        if pm.generic |> String.isNullOrWhiteSpace then
                            drug
                        else
                    ...
                    |> Array.tryFind (fun pm ->
                        ...
                        && drug.Doses
                           |> List.exists (fun dose ->
                               dose.Routes |> List.map _.Name |> List.exists (String.equalsCapInsens pm.route)
                           )
                    )
                    |> function
                        | None -> drug
                                    ...
                                    |> List.map (fun dose ->
                                        { dose with
                                            Routes =
                                                dose.Routes
                                                |> List.map (fun r ->
                                                    if pm.altRoute |> String.isNullOrWhiteSpace then
                                                        r
                                                    else
                                                        { r with
                                                            ProductRoute =
                                                                pm.altRoute |> String.toUpper |> String.trim
                                                        }
                                                )
                                        }
                                    )
