...
module ATCGroup =
    ...
    let parse gpks =
        query {
            for gpk in Zindex.BST711T.records () do
                join main in Zindex.BST801T.records () on (gpk.ATCODE.Substring(0, 1) = main.ATCODE.Trim())
                join ther in Zindex.BST801T.records () on (gpk.ATCODE.Substring(0, 3) = ther.ATCODE.Trim())
                join thes in Zindex.BST801T.records () on (gpk.ATCODE.Substring(0, 4) = thes.ATCODE.Trim())
                join phar in Zindex.BST801T.records () on (gpk.ATCODE.Substring(0, 5) = phar.ATCODE.Trim())
                join subs in Zindex.BST801T.records () on (gpk.ATCODE.Substring(0, 7) = subs.ATCODE.Trim())

                let form = Names.getThes gpk.GPKTVR Names.Form Names.Fifty

                let generic =
                    query {
                        for spk in Zindex.BST720T.records () |> Array.filter (fun r -> r.SPKODE = gpk.SPKODE) do
                            join ssk in Zindex.BST725T.records () on (spk.SSKODE = ssk.SSKODE)
                            join gngnk in Zindex.BST750T.records () on (ssk.GNSTAM = gngnk.GNGNK)
                            select gngnk.GNGNAM
                    }
                    |> Seq.toArray
                    |> Seq.fold (fun a s -> if a = "" then s else a + "/" + s) ""

                let route =

                    let rt =
                        Zindex.BST052T.records ()
                        |> Array.filter (fun r -> r.MUTKOD <> 1 && r.GPKODE = gpk.GPKODE)
                        |> Array.collect (fun r ->
                            Zindex.BST031T.records ()
                            |> Array.filter (fun r' -> r'.MUTKOD <> 1 && r'.PRKODE = r.PRKODE)
                        )
                        |> Array.collect (fun r ->
                    ...
                    if rt <> "" then
                        ...
                        match DoseRule.getGenericProducts () |> Array.tryFind (fun r -> r.Id = gpk.GPKODE) with
                        | Some p ->
    ...
    let _get () =
        ...
        fun () ->
            if (FilePath.groupCache useDemo) |> File.exists then
                FilePath.groupCache useDemo |> Json.getCache
            else
    ...
    let findByATC5 atc =
        get () |> Array.filter (fun g -> g.ATC5 |> String.equalsCapInsens atc)
                ...
                |> Array.collect (fun s ->
                    ...
                    |> Array.map (fun atc ->
                        {|
                            ...
                            FormQuantity =
                                gp.PrescriptionProducts
                                |> Array.fold (fun acc pp -> if pp.Quantity <> acc then pp.Quantity else acc) 1.0
