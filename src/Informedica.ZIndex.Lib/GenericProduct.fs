...
module GenericProduct =
    ...
    let getRoutes (gp: Zindex.BST711T.BST711T) =
        ...
        let rts =
            ...
            |> Array.filter (fun prp -> prp.MUTKOD <> 1 && prp.GPKODE = gp.GPKODE)
            |> Array.collect (fun prp ->
                Zindex.BST031T.records ()
                |> Array.filter (fun hp -> hp.MUTKOD <> 1 && hp.PRKODE = prp.PRKODE)
            )
    ...
    let getSubstances un (gp: Zindex.BST711T.BST711T) hpks =
        ...
        |> Array.filter (fun gs -> gs.GSKODE = gp.GSKODE && gs.MUTKOD <> 1)
        |> Array.collect (fun gs ->
            Zindex.BST750T.records ()
            |> Array.filter (fun gn -> gn.MUTKOD <> 1 && gs.GNNKPK = gn.GNGNK)
            |> Array.map (fun gn -> gs, gn)
        )
        |> Array.collect (fun (gs, gn) ->
            let stam = Zindex.BST750T.records () |> Array.find (fun s -> s.GNGNK = gn.GNSTAM)
            ...
            | _ ->
                ...
                |> Array.collect (fun hpk ->
                    ...
                    |> Array.filter (fun ig -> ig.HPKODE = hpk && ig.GNGNK = gn.GNGNK && ig.MUTKOD <> 1)
                    |> function
                        | igs when igs |> Array.isEmpty ->
                            Zindex.BST701T.records ()
                            |> Array.filter (fun ig -> ig.HPKODE = hpk && ig.GNSTAM = gn.GNSTAM && ig.MUTKOD <> 1)
                        | igs -> igs
                    |> Array.map (fun ig ->
    ...
    let getBarCodes (gp: GenericProduct) =
        ...
        |> Array.groupBy fst
        |> Array.map (fun (gpk, bc) -> gpk, bc |> Array.map snd)
    ...
    let findByBrand brand =
        ...
        |> Array.filter (fun gp ->
            ...
            |> Array.exists (fun pp ->
                pp.TradeProducts
                |> Array.exists (fun tp -> tp.Brand |> String.equalsCapInsens brand)
            )
        )
