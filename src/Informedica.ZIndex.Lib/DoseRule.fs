...
module DoseRule =
    ...
    let foldMinMax xs =
        ...
        |> Array.fold
            (fun
                ...
                (acc: RuleMinMax) ->
                {
                    Min = Option.min acc.Min min
                    Max = Option.max acc.Max max
                }
            )
            {
                Min = None
    ...
    let createProduct id nm : RuleProduct =
        {
            Id = id
            Name = nm
        }
    ...
    let createGenericProduct id nm rt un sl =
        {
            Id = id
            Name = nm
            Route = rt
    ...
    let createMinMax min max =
        let check = max |> string |> String.forall (fun c -> c = '9' || c = '.')
    ...
    let _getGenericProducts () =
        ...
        |> Array.map (fun gp ->
            let unt =
                gp.Substances
                |> Array.fold (fun acc s -> if acc = "" then s.FormUnit else acc) ""
    ...
    let _getPrescriptionProducts _ =
        query {
            for p in Zindex.BST052T.records () do
                join nm in Zindex.BST020T.records () on (p.PRNMNR = nm.NMNR)
                where (p.MUTKOD <> 1)
                select (createProduct p.PRKODE (nm.NMNAAM.Trim()))
        }
        |> Seq.toArray
    ...
    let _getTradeProducts _ =
        query {
            for p in Zindex.BST031T.records () do
                join nm in Zindex.BST020T.records () on (p.HPNAMN = nm.NMNR)
                where (p.MUTKOD <> 1)
                select (createProduct p.HPKODE (nm.NMNAAM.Trim()))
        }
        |> Seq.toArray
    ...
    /// This is a memoized function.
    /// </remarks>
    let getTradeProducts: unit -> RuleProduct[] = Memoization.memoize _getTradeProducts
    ...
    let getICPCRoute (icp: Zindex.BST642T.BST642T) =
        ...
        if r = "TOEDIENINGSWEG NIET INGEVULD" || r = "PARENTERAAL" then
            Array.empty
        else
            [| r |]
        |> Array.collect (fun r -> r |> String.splitAt ',' |> Array.map String.trim)
        |> Array.distinct
    ...
    let getDoseType (bas: Zindex.BST641T.BST641T) =
        ...
        |> Array.tryFind (fun tx -> tx.MUTKOD <> 1 && tx.TSNR = bas.GPDCTH && tx.TSITNR = bas.GPDCOD)
        |> (fun r -> if r |> Option.isNone then "" else r.Value.THNM50.Trim())


    /// Get the dose text for a BST642T record.
    let getICPCText (icp: Zindex.BST642T.BST642T) =
        Zindex.BST380T.records ()
        |> Array.tryFind (fun i -> i.ICPCNR1 = icp.ICPCNR1)
        |> (fun r -> if r.IsNone then "" else r.Value.ICPCTXT.Trim())


    /// Get the RuleFrequency for a BST643T record.
    let getFrequency (cat: Zindex.BST643T.BST643T) =
        Zindex.BST360T.records ()
        |> Array.tryFind (fun tx -> tx.MUTKOD <> 1 && tx.TTEHNR = cat.GPDFEE)
        |> (fun r -> if r |> Option.isNone then "" else r.Value.TTEHOM.Trim())
    ...
    let parse gpks =
        ...
        query {
            ...
            for dos in Zindex.BST649T.records () do
                ...
                join cat in Zindex.BST643T.records () on (dos.GPDDNR = cat.GPDDNR)
                // get many to 1 all dose indications
                join icp in Zindex.BST642T.records () on (cat.GPDCAT = icp.GPDCAT)
                // get many to 1 all prescription and trade products
                join bas in Zindex.BST641T.records () on (icp.GPDBAS = bas.GPDBAS)
                // get many to 1 all generic products
                join vas in Zindex.BST640T.records () on (bas.GPKODE = vas.GPKODE)
        ...
        |> Array.map (
            (fun (bas, r) ->
                let gpk, _, _ = bas
                let gpks = getGenericProducts () |> Array.filter (fun gp -> gp.Id = gpk)

                let rt =
                    if (r.Routes |> Array.isEmpty) then
                        gpks |> Array.collect _.Route
                    else
                        r.Routes
            ...
            >> (fun (bas, r) ->
                let _, prk, _ = bas
                let prks = getPresciptionProducts () |> Array.filter (fun pp -> pp.Id = prk)
                (bas, { r with PrescriptionProduct = prks })
            // Get trade products
            )
            >> (fun (bas, r) ->
                let _, _, hpk = bas
                let hpks = getTradeProducts () |> Array.filter (fun tp -> tp.Id = hpk)
    ...
    let toString2 (dr: DoseRule) =
        ...
        let minMaxToString u (mm: RuleMinMax) =
            let s =
                ...
                | None, Some max -> if max = 0. then "" else "tot " + (max |> string)
