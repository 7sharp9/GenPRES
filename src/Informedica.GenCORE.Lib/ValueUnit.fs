    ...
    module Measures =
        ...
        let inline toMeasure (group: Group.Group) brTo toMeasure u s vu =
            if vu |> getGroup = group then
                vu ==> u |> getValue |> Array.map (brTo >> toMeasure) |> Ok
            else
                $"cannot convert valueunit: {vu |> toStringEngShort} to {s}" |> Error


        let toDecimalCm =
            toMeasure Group.HeightGroup BigRational.toDecimal Conversions.cmFromDecimal Units.Height.centiMeter "cm"

        let toIntCm =
            toMeasure Group.HeightGroup BigRational.ToInt32 Conversions.cmFromInt Units.Height.centiMeter "cm"

        let toDecimalMeter =
            toMeasure Group.HeightGroup BigRational.toDecimal Conversions.meterFromDecimal Units.Height.meter "m"

        let toIntGram =
            toMeasure Group.WeightGroup BigRational.ToInt32 Conversions.gramFromInt Units.Weight.gram "gram"
        ...
        let toIntMonth =
            toMeasure Group.TimeGroup BigRational.ToInt32 Conversions.monthFromInt Units.Time.month "month"
    ...
    module Calculations =
        ...
        let inline calc2 vu1To vu2To resultTo calculation vu1 vu2 =
            result {
                ...
                return
                    ...
                    |> Array.map (fun (v1, v2) -> calculation (Some 2) v1 v2 |> resultTo)
            }
        ...
        let inline calc3 vu1To vu2To vu3To resultTo calculation vu1 vu2 vu3 =
            result {
                if [ vu1; vu2; vu3 ] |> List.forall isSingleValue |> not then
                    ...
                    return! calculation v1 v2 v3 |> resultTo |> Ok
            }
        ...
        let inline calc4 vu1To vu2To vu3To vu4To resultTo calculation vu1 vu2 vu3 vu4 =
            result {
                if [ vu1; vu2; vu3; vu4 ] |> List.forall isSingleValue |> not then
                    ...
                    return! calculation v1 v2 v3 v4 |> resultTo |> Ok
            }
        ...
        let inline calc4Opt vu1To vu2To vu3To vu4To resultTo calculation vu1 vu2 vu3 vu4 =
            result {
                if
                    [ vu1; vu2; vu3; vu4 ]
                    |> List.forall (Option.map isSingleValue >> (Option.defaultValue true))
                    |> not
                then
                    return! Error "value unit arguments must be all single value"
        ...
        module Age =
            ...
            let ageToString =
                ...
                let fYear =
                    Option.map (Measures.toIntYear >> Result.map (Array.map Some))
                    >> Option.defaultValue (Ok [| None |])

                let fMonth =
                    Option.map (Measures.toIntMonth >> Result.map (Array.map Some))
                    >> Option.defaultValue (Ok [| None |])

                let fWeek =
                    Option.map (Measures.toIntWeek >> Result.map (Array.map Some))
                    >> Option.defaultValue (Ok [| None |])

                let fDay =
                    Option.map (Measures.toIntDay >> Result.map (Array.map Some))
                    >> Option.defaultValue (Ok [| None |])

                calc4Opt fYear fMonth fWeek fDay id Calculations.Age.ageToString
            ...
            let ageToStringNL =
                ...
                calc4Opt fYear fMonth fWeek fDay id Calculations.Age.ageToStringNL
            ...
            let ageToStringNLShort =
                ...
                calc4Opt fYear fMonth fWeek fDay id Calculations.Age.ageToStringNlShort
        ...
        module BSA =
            ...
            let calcBSA method =
                calc2 Measures.toDecimalKg Measures.toDecimalCm Measures.fromDecimalM2 method
