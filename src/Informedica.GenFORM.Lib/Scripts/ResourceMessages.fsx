...
module ResourceAccumulator =
    ...
    let loadAllResourcesWithConfig (config: ResourceConfig) : Result<ResourceState, Message list> =
        try
            ...
            let unitMappings =
                config.GetUnitMappings()
                |> tryCollectLabeled "GetUnitMappings" acc

            let routeMappings =
                config.GetRouteMappings()
                |> tryCollectLabeled "GetRouteMappings" acc

            let validForms =
                config.GetValidForms()
                |> tryCollectLabeled "GetValidForms" acc

            let formRoutes =
                unitMappings
                |> Option.bind (fun um ->
                    config.GetFormRoutes um
                    |> tryCollectLabeled "GetFormRoutes" acc
                )

            let formularyProducts =
                config.GetFormularyProducts()
                |> tryCollectLabeled "GetFormularyProducts" acc

            let reconstitution =
                config.GetReconstitution()
                |> tryCollectLabeled "GetReconstitution" acc

            let parenteralMeds =
                unitMappings
                |> Option.bind (fun um ->
                    config.GetParenteralMeds um
                    |> tryCollectLabeled "GetParenteralMeds" acc
                )

            let enteralFeeding =
                unitMappings
                |> Option.bind (fun um ->
                    config.GetEnteralFeeding um
                    |> tryCollectLabeled "GetEnteralFeeding" acc
                )

            let products =
                match unitMappings, routeMappings, validForms, formRoutes, reconstitution, parenteralMeds, enteralFeeding, formularyProducts with
                | Some um, Some rm, Some vf, Some fr, Some rc, Some pm, Some ef, Some fp ->
                    config.GetProducts um rm vf fr rc pm ef fp
                    |> Some
                | _ -> None
            ...
            let doseRules =
                ...
                | Some rm, Some fr, Some prods ->
                    config.GetDoseRules rm fr prods
                    |> tryCollectLabeled "GetDoseRules" acc
                | _ -> None

            let solutionRules =
                match routeMappings, parenteralMeds, products with
                | Some rm, Some pm, Some prods ->
                    config.GetSolutionRules rm pm prods
                    |> tryCollectLabeled "GetSolutionRules" acc
                | _ -> None

            let renalRules =
                config.GetRenalRules()
                |> tryCollectLabeled "GetRenalRules" acc

            let allMessages = acc.ToArray()

            match unitMappings, routeMappings, validForms, formRoutes, formularyProducts,
                  reconstitution, parenteralMeds, enteralFeeding,
                  products, doseRules, solutionRules, renalRules with
            | Some um, Some rm, Some vf, Some fr, Some fp,
              Some rc, Some pm, Some ef,
              Some prods, Some dr, Some sr, Some rr ->
                Ok {
                    UnitMappings = um
                    RouteMappings = rm
                    ValidForms = vf
            ...
            | _ ->
                Error (allMessages |> Array.toList)
...
let tests =
    testList "ResourceAccumulator" [
        ...
        test "loadAllResourcesWithConfig sets Messages=[||] when all loaders succeed" {
            ...
            | Ok state ->
                state.Messages
                |> Expect.isEmpty "Messages should be empty when all succeed"
        }
        ...
        test "loadAllResourcesWithConfig collects messages from ALL failing loaders, not just the first" {
            ...
            let failRouteMappings () = Error [ Warning "route mapping warning" ]
            ...
            | Error msgs ->
                msgs
                |> List.length
                |> Expect.equal "both warnings should be collected" 2
                msgs
                |> List.exists (function Warning s -> s.Contains "route mapping" | _ -> false)
                |> Expect.isTrue "should contain route mapping warning"
                msgs
                |> List.exists (function Warning s -> s.Contains "renal rules" | _ -> false)
        ...
        test "multiple independent loader failures all appear in accumulated messages" {
            let failUnitMappings () = Error [ ErrorMsg ("unit mapping failure", None) ]
            ...
            | Ok _ -> failtest "Expected Error"
            | Error msgs ->
                msgs |> List.length |> Expect.equal "all three messages collected" 3
        }
    ]
