...
module FileDirectoryAgent =
    ...
    let private pruneOnce (dir: string) (pattern: string) (maxFiles: int) : Result<int, string> =
        try
            ...
            if maxFiles < 0 then
                invalidArg (nameof maxFiles) "maxFiles must be >= 0"
            ...
            if not (Directory.Exists fullDir) then
                ...
                let files =
                    try
                        Directory.EnumerateFiles(fullDir, pattern, SearchOption.TopDirectoryOnly)
                    with _ ->
                        seq { }
            ...
            let rec loop (state: State) =
                async {
                    ...
                    | SetPolicy(dir, maxFiles, pattern) ->
                        try
                            setPolicyInternal state dir maxFiles pattern
                        with ex ->
                            eprintfn $"FileDirectoryAgent SetPolicy error: %s{ex.Message}"

                        return! loop state

                    | RemovePolicy dir ->
                        try
                            removePolicyInternal state dir
                        with _ ->
                            ()
    ...
    let setPolicy dir maxFiles (agent: Agent<FileDirectoryMsg>) =
        SetPolicy(dir, maxFiles, None) |> agent.Post
        agent

    let setPolicyWithPattern dir maxFiles pattern (agent: Agent<FileDirectoryMsg>) =
        SetPolicy(dir, maxFiles, Some pattern) |> agent.Post
        agent
