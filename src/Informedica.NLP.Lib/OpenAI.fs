...
module OpenAI =
    ...
    type ResponseFormat = { ``type``: string }
    ...
    module Chat =
        ...
        let defaultChatInput model (msg: Message) (msgs: Message list) : ChatInput =
            ...
            {
                model = model
                messages = [ msg |> map ] |> List.append msgs
                tools = None
                max_tokens = 200
                temperature = 0.
                top_p = 1.0
...
"""
    ...
    let list () =
        Utils.get<Models> EndPoints.models apiKey
        |> Async.RunSynchronously
        |> function
            | Ok resp -> resp.Response.data |> List.map _.id
            | Error e -> [ e ]

    let completion (input: Completions.CompletionInput) =
        input
        |> JsonConvert.SerializeObject
        |> Utils.post<Completions.CompletionResponse> EndPoints.completions apiKey


    let chat (input: Chat.ChatInput) =
        input
        |> JsonConvert.SerializeObject
        |> Utils.post<Chat.ChatResponse> EndPoints.chat apiKey


    let chatJson (input: Chat.ChatInput) =
        { input with response_format = { ``type`` = "json_object" } }
        |> JsonConvert.SerializeObject
        |> Utils.post<Chat.ChatResponse> EndPoints.chat apiKey
        ...
        async {
            let rec validateLoop reTry (input: Chat.ChatInput) =
                async {
                    ...
                    match resp with
                    | Ok result ->
                        let answer = result.Response.choices |> List.last
                                        ...
                                        messages =
                                            [
                                                {|
                                                    role = "user"
                                                    content =
                                                        $"The answer: {answer.message.content} was not correct because of %s{err}. Please try again answering:\n\n%s{original.content}"
                                                |}
                                            ]
                                            |> List.append input.messages
                                    }
                                    ...
                                    messages =
                                        [
                                            {|
                                                role = "user"
                                                content =
                                                    $"There was an error: %s{err}. Please try again answering:\n\n%s{original.content}"
                                            |}
                                        ]
                                        |> List.append input.messages
                                }
