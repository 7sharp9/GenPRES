...
- **Scripts (NLP)**: Overhaul NLP dose-rule extraction pipeline — replace `DoseRuleExtraction.fsx` (560 lines) with `DoseRuleExtract.fsx` (1 952 lines); add `DoseRuleExtractInteractiveDemo.fsx` (261 lines) for step-by-step interactive exploration; significantly expand `docs/data-extraction/doserule-extraction-prompt.md` (+208/−114 lines) with improved LLM prompting strategy, structured JSON output schema, and field-level extraction guidance; update `DoseRuleTests.fsx` and `DoseRuleValidation.fsx` with revised test cases (PR #317)
